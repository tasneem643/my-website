...
def get_app_connection():
    return pymysql.connect(
        ...
        database=os.environ.get('app_NAME'),
...
@app.route('/admin/delete_teacher/<int:t_id>')
...
def get_app_connection():
    if os.environ.get('MUSAID_ENV') == 'production' or os.environ.get('DATABASE_URL'):
        return pymysql.connect(
            ...
            password=os.environ.get('app_PASSWORD', ''),
            database=os.environ.get('app_NAME', 'musaid_ist'),
            cursorclass=pymysql.cursors.DictCursor
        )
    else:
        return app.connect(DATABASE)

def ensure_schema():
    conn = get_app_connection()
    try:
        if os.environ.get('MUSAID_ENV') == 'production' or app.is_mysql():
            cursor = conn.cursor()
            cursor.execute(
